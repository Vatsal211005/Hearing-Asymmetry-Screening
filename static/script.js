...
async function showResultsScreen(data) {
    ...
    const leftAvg = data.left_avg || 0;
    const rightAvg = data.right_avg || 0;
    const maxDiff = data.max_diff || 0;

    logDebug(`maxDiff: ${maxDiff}, leftAvg: ${leftAvg}, rightAvg: ${rightAvg}`);
    logDebug(`Thresholds: ${JSON.stringify(thresholds)}`);

    // Validate maxDiff by recalculating client-side
    let calculatedMaxDiff = 0;
    testFrequencies.forEach(freq => {
        const left = thresholds.left[freq] !== undefined ? thresholds.left[freq] : 40;
        const right = thresholds.right[freq] !== undefined ? thresholds.right[freq] : 40;
        const diff = Math.abs(left - right);
        calculatedMaxDiff = Math.max(calculatedMaxDiff, diff);
        logDebug(`Client-side check - Freq ${freq} Hz: Left=${left}, Right=${right}, Diff=${diff}`);
    });
    if (calculatedMaxDiff !== maxDiff) {
        console.warn(`Server maxDiff (${maxDiff}) differs from client calculation (${calculatedMaxDiff})`);
        alert('Warning: Asymmetry calculation may be inconsistent. Check server logs.');
    }
    ...
    testFrequencies.forEach(freq => {
        ...
        if (diff !== '-' && parseFloat(diff) >= 20) {
            logDebug(`Asymmetry at ${freq} Hz: Left=${left}, Right=${right}, Diff=${diff}`);
        }
    });
